...
# todo ny=42, 49, 54, 55, chi=77
iimin <- 1
iimax = nrow( dfusa )
iimax <- 1
res <- POST("https://www.zagat.com/r/washington-dc")
...
for(ii in iimin:iimax ) {
    ...
    if ( nb < 300000 ) {
       ...
       #query <- paste0("https://www.zagat.com/r/komi-washington")
       #query <- "https://www.zagat.com/search/chicago?text=Goosefoot&currentcity=Washington&city=Washington&rdr=1"
       download.file( query , destfile = YcacheName , method="curl")
       html.raw<-htmlTreeParse( YcacheName, useInternalNodes=T )
...
#mesEtoiles <- cbind( mesEtoiles, ndf$type )
...
write.csv( stared , file = "/Users/bdolimier/persodev/michelinDC/michelin2016_result.csv", row.names=TRUE , quote=FALSE )
```
